...
static NSString *const VERSION = @"2.3.0";
...
- (NSString *)sanitizedTag;
- (NSString *)sanitizedThread;
- (NSString *)sanitizedMessage;
@end
...
- (NSString *)fullMessageWithAppVersion:(NSString *)appVersion {
    ...
    [str appendString:@" - "];
    [str appendString:[self sanitizedThread]];
    [str appendString:@" ["];
    [str appendString:[self sanitizedTag]];
    [str appendString:@"] "];
    [str appendString:[self levelString]];
    [str appendString:@" "];
    [str appendString:[self sanitizedMessage]];
    [str appendString:@"\n"];
    return str;
}

- (NSString *)sanitizedTag {
    if (!self.tag || self.tag.length == 0) return @"(null)";
    return self.tag;
}

- (NSString *)sanitizedThread {
    if (!self.thread || self.thread.length == 0) return @"(null)";
    return [self.thread stringByReplacingOccurrencesOfString:@" " withString:@"_"];
}
            ...
            sUploadBlock = ^{
                if ([self isConnected]) {
                    ...
                    for (NSString *file in files) {
                        ...
                        if (error) {
                            if ([self getLoggerLogsAllowed]) OULogError(TAG, @"Could not upload file.", error);
                        } else {
                            ...
                            if (error) {
                                if ([self getLoggerLogsAllowed]) OULogError(TAG, @"Could not build json.", error);
                                error = nil;
                            } else {
...
+ (void)logInternal:(NSNumber *)level tag:(NSString *)tag message:(NSString *)message {
    ...
    OULogInternal *log = [[OULogInternal alloc] initWithLocation:[self getLocation]
    ...
    } else if ([self getBuffered] && level.integerValue < OULogLevelError) {
        [sQueue addObject:log];
...
+ (void)processLog:(OULogInternal *)log {
    ...
    NSString *fullMessage = [log fullMessageWithAppVersion:[self getAppVersion]];
    
    if (![self getDiskOnly]) {
        NSLog(@"%@ [%@] %@ %@", [log sanitizedThread], [log sanitizedTag], [log levelString], [log sanitizedMessage]);
    }
    
    if (sFileHandle) {
        [sFileHandle seekToEndOfFile];
        
        NSString *encrypted = [self encrypt:fullMessage];
        if (encrypted) {
...
+ (void)updateFiles {
    ...
    if (error) {
        if ([self getLoggerLogsAllowed]) OULogError(TAG, @"Could not update files.", error);
    } else {
...
+ (BOOL)removeFile:(NSString *)file {
    ...
    if (error) {
        if ([self getLoggerLogsAllowed]) OULogError(TAG, @"Could not remove file.", error);
        return NO;
    } else {
        return YES;
    }
...
+ (BOOL)copyFile:(NSString *)original to:(NSString *)toPath {
    ...
    if (error) {
        if ([self getLoggerLogsAllowed]) OULogError(TAG, @"Could not copy file.", error);
        return NO;
    } else {
        return YES;
    }
...
+ (void)createDirectory:(NSString *)directory {
    if (![sFileManager fileExistsAtPath:directory isDirectory:nil]) {
        ...
        if (error && [self getLoggerLogsAllowed]) OULogError(TAG, @"Could not create directory.", error);
    }
}
...
+ (NSMutableArray *)listFiles:(NSString *)dirPath {
    ...
    if (error) {
        if ([self getLoggerLogsAllowed]) OULogError(TAG, @"Could not list files.", error);
    } else {
        for (NSString *file in files) {
...
+ (NSString *)encrypt:(NSString *)input {
    ...
    return [[data base64EncodedStringWithOptions:0] stringByAppendingString:@"\n"];
}
